...
With {\em DAG}, windows are distributed to workers dynamically.
...
Fig.~\ref{faceRecognition} shows that {\em DAG} outperforms {\em CUDA-Static Warp Scheduling} by 1.35$\times$.
All the performance improvement can be attributed to the capability of balancing computations among SMs of the GPU.
The {\em DAG} version achieves 90 percent of the hand-optimized version at substantially less programming complexity.
...
However, it performs a non-blocking kernel launch with a CUDA stream, allowing the runtime to schedule ready tasks while servicing communication for other tasks.
...
Fig.~\ref{fig:stencil_single_nodes} shows the strong scaling performance of four code variants on single compute nodes: a) consists of six Kepler GPUs connected via a shared PCIe bus, and b) consists of four Pascal GPUs split between two sockets, each connected to two GPUs with NVLink.
